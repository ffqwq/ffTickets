<template>
    <div>
        ...
        <div>
            <el-table :data="filtedSchedules" style="width: 100%" height="550">
                ...
                <el-table-column prop="editAndSave" label="编辑" > 
                    <!-- 固定用法，读取表格元素时使用scope -->
                    <template slot-scope="scope">
                        <el-button @click.native.prevent="editAndSaveSchedule(scope.$index,filtedSchedules)">编辑</el-button>
                    </template>
                </el-table-column>
                ...
                <el-table-column prop="delete" label="删除" > 
                    <!-- 固定用法，读取表格元素时使用scope -->
                    <template slot-scope="scope">
                        <el-button @click.native.prevent="deleteSchedule(filtedSchedules[scope.$index].flight_id,filtedSchedules[scope.$index].start_date)">删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </div>
        ...
        <div>
            <el-button @click="insertSchedule">添加调度</el-button>
        </div>
        ...
        <div>
            <schedules-dialog ref='schedulesDialog'></schedules-dialog>
        </div>
...
import schedulesDialog from '@/components/admin/dialogs/schedulesDialog.vue';
export default{
    data(){
        return{
            schedules:[],
    ...
    methods:{
        ...
        editAndSaveSchedule(index,schedules){
            this.$refs.schedulesDialog.dialogVisible=true;
            this.$refs.schedulesDialog.insertOrEdit = "edit";

            this.$refs.schedulesDialog.oldFlight_id=schedules[index].flight_id;
            this.$refs.schedulesDialog.oldStart_date=schedules[index].start_date;

            this.$refs.schedulesDialog.dialogData=schedules[index];
        },
        insertSchedule(){
            this.$refs.schedulesDialog.dialogVisible=true;
            this.$refs.schedulesDialog.dialogData={};
            this.$refs.schedulesDialog.insertOrEdit = "insert";
        },
        deleteSchedule(flight_id,start_date){

            this.$confirm('确定要删除此调度吗？', '提示', {
            ...
            .then(() => {
                request({
                    method:'delete',
                    url:'/schedules/deleteSchedule?flight_id='+flight_id+'&start_date='+start_date,
                    headers:{
                        'token':localStorage.getItem('tickets_token'),
                        'user_type':localStorage.getItem('tickets_user_type')
                    }
